...
// Modèle de parking vide, à modifier une fois le modèle JSON construit
function ParkingModel(){ // quand cette fonction est appelée, elle renvoie simplement un objet parking vide "modèle" 
    return {
        "id": parseInt(mesParkings.PARKINGS[mesParkings.PARKINGS.length-1]._ID)+1, // permet d'incrémenter automatiquement l'ID par rapport au dernier ID de la BDD
        "name": "", // Valeur chaîne vide
        "address": "", // Valeur chaîne vide
        "capacity": 0, // valeur int vide
    };
}
